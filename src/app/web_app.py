...
from src.agent.chatbot import VinpearlBaselineChatbot
...
def build_agent() -> VinpearlBaselineChatbot:
    if load_dotenv:
        load_dotenv(PROJECT_ROOT / ".env")

    provider = os.getenv("BASELINE_PROVIDER", "local").lower()
    ...
    if provider == "openai" and api_key:
        ...
        return VinpearlBaselineChatbot(llm=OpenAIProvider(model_name=model_name, api_key=api_key))

    return VinpearlBaselineChatbot()
...
<head>
  ...
  <title>Vinpearl Room Tool Chatbot</title>
  <style>
    :root {
      --bg: #f6f7f9;
      --panel: #ffffff;
...
<body>
  <div class="app">
    ...
    <main>
      <header class="topbar">
        <div>
          <strong>Tư vấn phòng Vinpearl</strong>
          <p class="subtle">Local/OpenAI model kết nối 2 tool tìm cơ sở và kiểm tra phòng.</p>
        </div>
  ...
  <script>
    ...
    function addMessage(role, text, payload = {}) {
      ...
      if (payload.trace_text) {
        ...
        details.innerHTML = `<summary>Tool trace</summary><pre>${escapeHtml(payload.trace_text)}</pre>`;
    ...
    async function sendMessage(text = "") {
      ...
      try {
        ...
        addMessage("assistant", "Không gọi được chatbot nội bộ. Vui lòng kiểm tra server.");
      } finally {
...
class VinpearlRequestHandler(BaseHTTPRequestHandler):
    ...
    @classmethod
    def get_agent(cls) -> VinpearlBaselineChatbot:
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/locations":
            ...
            self._send_json(
                {
                    "hotels": agent.locations_for_ui(),
                    "date_range": {
                        "from": agent.kb.min_date.isoformat(),
                        "to": agent.kb.max_date.isoformat(),
                    },
                }
            )
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            self._send_json(self.get_agent().respond(message, context=context))
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=500)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Vinpearl room tool chatbot UI.")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8765)
    args = parser.parse_args()

    server = ThreadingHTTPServer((args.host, args.port), VinpearlRequestHandler)
    print(f"Vinpearl Room Tool Chatbot running at http://{args.host}:{args.port}")
